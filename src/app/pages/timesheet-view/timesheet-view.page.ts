...
// import { TimesheetFormPage } from '../timesheet-form/timesheet-form.page';
@Component({
  selector: 'app-timesheet-view',
  templateUrl: './timesheet-view.page.html',
  styleUrls: ['./timesheet-view.page.scss'],
  standalone:false
})
export class TimesheetViewPage implements OnInit {
  ...
  NewRc
  tsentrylength
  constructor(private loadingController: LoadingController, public router: Router, private navParams: NavParams,
    private route: ActivatedRoute, private webApi: WebApiService, private toastCtrl: ToastController,
    private modalCtrl: ModalController, public alertController: AlertController,private menuCtrl: MenuController) {
    // this.date = navParams.get('date')
    // this.timesheetid = navParams.get('timesheetid')
    // this.NewRc = navParams.get('NewDate1')
    // console.log("--------------------------------------------")
  ...
  async getData() {
    ...
    await this.route.queryParams.subscribe(async params => {
      if (this.router.getCurrentNavigation().extras.state) {
       const data = await this.router.getCurrentNavigation().extras.state;
    ...
    await this.webApi.getData("/timesheetservice/gettimesheet", Params).then(async data => {
      ...
      if (returnData.issucess === 'Y') {
        ...
        // console.log(this.tsentrylength);
  ...
  async Waring() {
    let alert = await this.alertController.create({
      ...
      header: ' Warnning!!!',
      message:'You are not filling the daily timesheet. Make sure to fill the timesheet daily base or it will affect your annual performance.',
   // subTitle: text,
    buttons: [{
    text: 'OK',
      handler: () => {
